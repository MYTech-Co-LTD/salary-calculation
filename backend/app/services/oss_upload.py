...
ALLOWED_EXTS = (".xlsx", ".xls")  # 与前端 DropZone accept 及 python-calamine 解析器选择一致


def upload_key(month: str, kind: str, ext: str) -> str:
    """生成一次性上传 key：{prefix}uploads/{month}/{kind}-{uuid}{ext}。

    ext 随真实文件后缀（.xlsx/.xls）——真 BIFF .xls 存成 .xlsx 会让
    python-calamine 选错解析器导致导入 500，故扩展名必须贯穿。"""
    if ext not in ALLOWED_EXTS:
        raise ValueError(f"ext 必须是 {ALLOWED_EXTS} 之一，实际 {ext!r}")
    return f"{_env('OSS_PREFIX', 'salary/')}uploads/{month}/{kind}-{uuid4().hex}{ext}"
...
def key_matches(key: str, month: str, kind: str) -> bool:
    ...
    if not key.startswith(prefix):
        return False
    return bool(re.fullmatch(r"[0-9a-f]{32}\.(xlsx|xls)", key[len(prefix):]))
...
def fetch_to_file(key: str, local_path: str) -> None:
    """内网拉取对象写本地文件（原子落盘）。finally 总删对象：中转用完即弃，
    失败也不留垃圾（重试 = 前端重新走 ticket 整段上传）。

    先写 .part 再 os.replace——中断时不会就地覆写同月历史存档。"""
    upload, _ = _clients()
